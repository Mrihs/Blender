...
![Abbildung 1: Vertices, Edges und Faces](images/Chapter10/structure.png){width=25%}
...
![Abbildung 2: Buttons für den Vertice- Edge- und Face-Select-Modus in der linken oberen Ecke des Edit-Modus.](images/Chapter10/select_icons.png){width=25%}
...
![Abbildung 8: Der Standardwürfel bestehend aus Tris statt aus Quads.](images/Chapter10/tris.png){width=25%}
...
Abhängig davon, welcher Select-Mode gerade aktiviert ist, lassen sich entweder die Vertices, die Edges oder Faces auswählen. Die Auswahl der jeweiligen Elemente geschieht mittels der linken Maustaste. Wie auch im Object-Mode können mittels gedrückter ƒ Taste zusätzliche Elemente ausgewählt werden. Ebenso kann mit der Taste C der Circle-Select-Mode und mittels der Taste B der Box-Select-Mode verwendet werden. Zudem kann mit der Tastenkombination ≈ + I die Auswahl umgekehrt werden.

<div class="marginnote"> Select linked mittels L </div>

Im Edit-Modus gibt es zudem noch einige zusätzliche Auswahl-Optionen. Wenn sich die Maus über einem Element eines Objekts befindet kann mit der Taste L beispielsweise alle Elemente, welche direkt über Edges damit verbunden sind ausgewählt werden. Dies ermöglich es, alle verbundenen Elemente auszuwählen. Um auch unverbundene Elemente eines Objektes auszuwählen kann die Taste A gedrückt werden.

<div class="marginnote"> Edge-Loop-Select bei gedrückter Taste ¥ </div>

Zudem lassen sich auch ganze Verbindungen von Edges auswählen, wenn die Taste ¥ während der Auswahl gedrückt wird. Dadurch werden alle Edges, die gemeinsam eine Linie bilden ausgewählt. Dies ist auch im Vertex Select-Modus möglich. Im Face-Select-Modus werden alle Faces, die gemeinsam eine Linie bilden ausgewählt (hierfür muss der Mausklick allerdings bei einem Edge erfolgen – nicht in der Fläche des Faces).
...
Wenn alle Faces innerhalb eines Objekts ausgewählt werden, mittels der Taste A, kann anschliessend mittels der Tastenkombination ¥ + N das Normals-Menü bei der Maus geöffnet werden. Unter der Option «Flip» können anschliessend alle Normals in die umgekehrte Richtung gekehrt werden. Zudem können die Normals hier auch mittels der Option «Recalculate Outside» zur äusseren Seite hin berechnet werden oder mittels «Reculaculate Inside» zur inneren Seite hin berechnet werden.
